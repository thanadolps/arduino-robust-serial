...
Order read_order(SoftwareSerial& serial)
{
	return (Order) serial.read();
}

void wait_for_bytes(SoftwareSerial& serial, int num_bytes, unsigned long timeout)
{
	unsigned long startTime = millis();
	//Wait for incoming bytes or exit if timeout
	while ((serial.available() < num_bytes) && (millis() - startTime < timeout)){}
...
// NOTE : Serial.readBytes is SLOW
// this one is much faster, but has no timeout
void read_signed_bytes(SoftwareSerial& serial, int8_t* buffer, size_t n)
{
	size_t i = 0;
	int c;
	while (i < n)
...
int8_t read_i8(SoftwareSerial& serial)
{
	wait_for_bytes(1, 100); // Wait for 1 byte with a timeout of 100 ms
  return (int8_t) serial.read();
}

int16_t read_i16(SoftwareSerial& serial)
{
  int8_t buffer[2];
	wait_for_bytes(2, 100); // Wait for 2 bytes with a timeout of 100 ms
	read_signed_bytes(buffer, 2);
...
int32_t read_i32(SoftwareSerial& serial)
{
  int8_t buffer[4];
	wait_for_bytes(4, 200); // Wait for 4 bytes with a timeout of 200 ms
	read_signed_bytes(buffer, 4);
...
void write_order(SoftwareSerial& serial, enum Order myOrder)
{
	uint8_t* Order = (uint8_t*) &myOrder;
  serial.write(Order, sizeof(uint8_t));
}

void write_i8(SoftwareSerial& serial, int8_t num)
{
  serial.write(num);
}

void write_i16(SoftwareSerial& serial, int16_t num)
{
	int8_t buffer[2] = {(int8_t) (num & 0xff), (int8_t) (num >> 8)};
  serial.write((uint8_t*)&buffer, 2*sizeof(int8_t));
}

void write_i32(SoftwareSerial& serial, int32_t num)
{
	int8_t buffer[4] = {(int8_t) (num & 0xff), (int8_t) (num >> 8 & 0xff), (int8_t) (num >> 16 & 0xff), (int8_t) (num >> 24 & 0xff)};
  serial.write((uint8_t*)&buffer, 4*sizeof(int8_t));
}